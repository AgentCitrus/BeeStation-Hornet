...
/datum/species/proc/body_temperature_damage(mob/living/carbon/human/humi)
	...
	if(humi.coretemperature > bodytemp_heat_damage_limit && !HAS_TRAIT(humi, TRAIT_RESISTHEAT))
		...
		burn_damage = burn_damage * heatmod * humi.physiology.heat_mod * (HAS_TRAIT(humi, TRAIT_HEATWEAKNESS) ? 2 : 1)
	...
	if(humi.coretemperature < bodytemp_cold_damage_limit && !HAS_TRAIT(humi, TRAIT_RESISTCOLD))
		var/weakness_modifier = HAS_TRAIT(humi, TRAIT_COLDWEAKNESS) ? 2 : 1
		switch(humi.coretemperature)
			if(201 to INFINITY)
				humi.apply_damage(COLD_DAMAGE_LEVEL_1 * coldmod * humi.physiology.cold_mod * weakness_modifier, BURN)
			if(120 to 200)
				humi.apply_damage(COLD_DAMAGE_LEVEL_2 * coldmod * humi.physiology.cold_mod * weakness_modifier, BURN)
			else
				humi.apply_damage(COLD_DAMAGE_LEVEL_3 * coldmod * humi.physiology.cold_mod * weakness_modifier, BURN)
	...
	switch(adjusted_pressure)
		...
		if(HAZARD_HIGH_PRESSURE to INFINITY)
			if(!HAS_TRAIT(H, TRAIT_RESISTHIGHPRESSURE))
				H.adjustBruteLoss(min(((adjusted_pressure / HAZARD_HIGH_PRESSURE) -1 ) * PRESSURE_DAMAGE_COEFFICIENT, MAX_HIGH_PRESSURE_DAMAGE) * H.physiology.pressure_mod * (HAS_TRAIT(H, TRAIT_HIGHPRESSUREWEAKNESS) ? 2 : 1))
		...
		// Very low pressure, show an alert and take damage
			...
			if(HAS_TRAIT(H, TRAIT_RESISTLOWPRESSURE))
				...
				H.adjustBruteLoss(LOW_PRESSURE_DAMAGE * H.physiology.pressure_mod * (HAS_TRAIT(H, TRAIT_LOWPRESSUREWEAKNESS) ? 2 : 1))
