...
/mob/living/carbon/monkey/handle_environment(datum/gas_mixture/environment)
	...
	if(bodytemperature > BODYTEMP_HEAT_DAMAGE_LIMIT && !HAS_TRAIT(src, TRAIT_RESISTHEAT))
		...
		var/weakness_modifier = (HAS_TRAIT(src, TRAIT_HEATWEAKNESS) ? 2 : 1)
		switch(bodytemperature)
			if(360 to 400)
				throw_alert("temp", /atom/movable/screen/alert/hot, 1)
				apply_damage(HEAT_DAMAGE_LEVEL_1 * weakness_modifier, BURN)
			if(400 to 460)
				throw_alert("temp", /atom/movable/screen/alert/hot, 2)
				apply_damage(HEAT_DAMAGE_LEVEL_2 * weakness_modifier, BURN)
			if(460 to INFINITY)
				throw_alert("temp", /atom/movable/screen/alert/hot, 3)
				if(on_fire)
					apply_damage(HEAT_DAMAGE_LEVEL_3 * weakness_modifier, BURN)
				else
					apply_damage(HEAT_DAMAGE_LEVEL_2 * weakness_modifier, BURN)
		...
		if(!istype(loc, /obj/machinery/atmospherics/components/unary/cryo_cell))
			...
			var/weakness_modifier = (HAS_TRAIT(src, TRAIT_COLDWEAKNESS) ? 2 : 1)
			switch(bodytemperature)
				if(200 to BODYTEMP_COLD_DAMAGE_LIMIT)
					throw_alert("temp", /atom/movable/screen/alert/cold, 1)
					apply_damage(COLD_DAMAGE_LEVEL_1 * weakness_modifier, BURN)
				if(120 to 200)
					throw_alert("temp", /atom/movable/screen/alert/cold, 2)
					apply_damage(COLD_DAMAGE_LEVEL_2 * weakness_modifier, BURN)
				if(-INFINITY to 120)
					throw_alert("temp", /atom/movable/screen/alert/cold, 3)
					apply_damage(COLD_DAMAGE_LEVEL_3 * weakness_modifier, BURN)
		else
			clear_alert("temp")
